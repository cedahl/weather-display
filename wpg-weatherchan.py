...
def weather_page(PageColour, PageNum):
    ...
    if (PageNum > 0):
        ...
        local_tz = datetime.datetime.now(tz=pytz.utc).astimezone(timezone('US/Pacific'))

        # weather data
        print(weather_laca)
        print("winnipeg condition " + ec_en_wpg.conditions["condition"]["value"])
        print("winnipeg tendency " + ec_en_wpg.conditions["tendency"]["value"])

        # temp_cur = str(ec_en_wpg.conditions["temperature"]["value"])
        temp_cur = str(to_fahrenheit(weather_laca.temperature))
        # temp_high = str(ec_en_wpg.conditions["high_temp"]["value"])
        temp_high = str(to_fahrenheit(weather_laca.max_temperature_last_24_hours))
        # temp_low = str(ec_en_wpg.conditions["low_temp"]["value"])
        temp_low = str(to_fahrenheit(weather_laca.min_temperature_last_24_hours))
        # humidity = str(ec_en_wpg.conditions["humidity"]["value"])
        humidity = str(weather_laca.relative_humidity)
        # condition = ec_en_wpg.conditions["condition"]["value"]
        condition = str(weather_laca.text_description)
        # pressure = str(ec_en_wpg.conditions["pressure"]["value"])
        pressure = str(weather_laca.barometric_pressure)
        tendency = ec_en_wpg.conditions["tendency"]["value"]
        dewpoint = str(to_fahrenheit(weather_laca.dewpoint))
        # uvi_value = uv_laca.entries[1]["UV_VALUE"]
        #todo fix
        uvi_value = 0
        uv_index = str(to_fahrenheit(uvi_value))

        for a in uv_laca.entries:
            print("wawaaa   " + a)


        '''uv_index = str(ec_en_wpg.conditions["uv_index"]["value"]) if ec_en_wpg.conditions["uv_index"] and \
                                                                     ec_en_wpg.conditions["uv_index"][
                                                                         "value"] != None else "--"
        pop = str(ec_en_wpg.conditions["pop"]["value"]) if ec_en_wpg.conditions["pop"] and ec_en_wpg.conditions["pop"][
            "value"] != None else "0"'''
        pop = str(0.3)
        # todo fix pop

        # check severity of uv index
        if uvi_value is not None:
            if uvi_value <= 2:
                uv_cat = "LOW"
            elif 2 < uvi_value <= 5:
                uv_cat = "MODERT"
        ...
        # check if windchill or heat index is present, if neither leave area blank
        if weather_laca.wind_chill > 0:
            windchill = str(to_fahrenheit(weather_laca.wind_chill))
            windchildex = "WIND CHILL " + windchill + " F"
        elif weather_laca.heat_index > 0:
            heatindex = str(to_fahrenheit(weather_laca.heat_index))
            windchildex = "HEAT INDEX " + heatindex + " F"
        else:
            windchildex = ""
        ...
        s1 = ("LOS ANGELES " + real_forecast_time + " " + str(local_tz) + "  " + real_forecast_date.upper()).center(35,
                                                                                                                 " ")
    ...
    '''
        ...
        wsum_day2 = textwrap.wrap(ec_en_wpg.daily_forecasts[1]["period"].upper() + ".." + ec_en_wpg.daily_forecasts[1][
            "text_summary"].upper(), 35)
        wsum_day3 = textwrap.wrap(ec_en_wpg.daily_forecasts[2]["period"].upper() + ".." + ec_en_wpg.daily_forecasts[2][
            "text_summary"].upper(), 35)
        wsum_day4 = textwrap.wrap(ec_en_wpg.daily_forecasts[3]["period"].upper() + ".." + ec_en_wpg.daily_forecasts[3][
            "text_summary"].upper(), 35)
        wsum_day5 = textwrap.wrap(ec_en_wpg.daily_forecasts[4]["period"].upper() + ".." + ec_en_wpg.daily_forecasts[4][
            "text_summary"].upper(), 35)
        wsum_day6 = textwrap.wrap(ec_en_wpg.daily_forecasts[5]["period"].upper() + ".." + ec_en_wpg.daily_forecasts[5][
            "text_summary"].upper(), 35)
        ...
        if ("value" in ec_en_wpg.conditions["high_temp_yesterday"] and ec_en_wpg.conditions["high_temp_yesterday"][
            "value"] != None):
            temp_yest_high = str(round(ec_en_wpg.conditions["high_temp_yesterday"]["value"]))
        else:
            temp_yest_high = ""

        if ("value" in ec_en_wpg.conditions["low_temp_yesterday"] and ec_en_wpg.conditions["low_temp_yesterday"][
            "value"] != None):
        ...
        cond_brn = (ec_en_brn.conditions["condition"]["value"]) if (
                    "value" in ec_en_brn.conditions["condition"] and ec_en_brn.conditions["condition"][
                "value"] != None) else " "
        cond_thm = (ec_en_thm.conditions["condition"]["value"]) if (
                    "value" in ec_en_thm.conditions["condition"] and ec_en_thm.conditions["condition"][
                "value"] != None) else " "
        cond_tps = (ec_en_tps.conditions["condition"]["value"]) if (
                    "value" in ec_en_tps.conditions["condition"] and ec_en_tps.conditions["condition"][
                "value"] != None) else " "
        cond_fln = (ec_en_fln.conditions["condition"]["value"]) if (
                    "value" in ec_en_fln.conditions["condition"] and ec_en_fln.conditions["condition"][
                "value"] != None) else " "
        cond_chu = (ec_en_chu.conditions["condition"]["value"]) if (
                    "value" in ec_en_chu.conditions["condition"] and ec_en_chu.conditions["condition"][
                "value"] != None) else " "
        cond_ken = (ec_en_ken.conditions["condition"]["value"]) if (
                    "value" in ec_en_ken.conditions["condition"] and ec_en_ken.conditions["condition"][
                "value"] != None) else " "
        cond_tby = (ec_en_tby.conditions["condition"]["value"]) if (
                    "value" in ec_en_tby.conditions["condition"] and ec_en_tby.conditions["condition"][
                "value"] != None) else " "
        ...
        cond_vic = (ec_en_vic.conditions["condition"]["value"]) if (
                    "value" in ec_en_vic.conditions["condition"] and ec_en_vic.conditions["condition"][
                "value"] != None) else " "
        cond_van = (ec_en_van.conditions["condition"]["value"])[0:13] if (
                    "value" in ec_en_van.conditions["condition"] and ec_en_van.conditions["condition"][
                "value"] != None) else " "
        cond_edm = (ec_en_edm.conditions["condition"]["value"])[0:13] if (
                    "value" in ec_en_edm.conditions["condition"] and ec_en_edm.conditions["condition"][
                "value"] != None) else " "
        cond_cal = (ec_en_cal.conditions["condition"]["value"])[0:13] if (
                    "value" in ec_en_cal.conditions["condition"] and ec_en_cal.conditions["condition"][
                "value"] != None) else " "
        cond_ssk = (ec_en_ssk.conditions["condition"]["value"])[0:13] if (
                    "value" in ec_en_ssk.conditions["condition"] and ec_en_ssk.conditions["condition"][
                "value"] != None) else " "
        cond_reg = (ec_en_reg.conditions["condition"]["value"])[0:13] if (
                    "value" in ec_en_reg.conditions["condition"] and ec_en_reg.conditions["condition"][
                "value"] != None) else " "
        cond_wht = (ec_en_wht.conditions["condition"]["value"])[0:13] if (
                    "value" in ec_en_wht.conditions["condition"] and ec_en_wht.conditions["condition"][
                "value"] != None) else " "
        ...
        cond_tor = (ec_en_tor.conditions["condition"]["value"]) if (
                    "value" in ec_en_tor.conditions["condition"] and ec_en_tor.conditions["condition"][
                "value"] != None) else " "
        cond_otw = (ec_en_otw.conditions["condition"]["value"]) if (
                    "value" in ec_en_otw.conditions["condition"] and ec_en_otw.conditions["condition"][
                "value"] != None) else " "
        cond_qbc = (ec_en_qbc.conditions["condition"]["value"]) if (
                    "value" in ec_en_qbc.conditions["condition"] and ec_en_qbc.conditions["condition"][
                "value"] != None) else " "
        cond_mtl = (ec_en_mtl.conditions["condition"]["value"]) if (
                    "value" in ec_en_mtl.conditions["condition"] and ec_en_mtl.conditions["condition"][
                "value"] != None) else " "
        cond_frd = (ec_en_frd.conditions["condition"]["value"]) if (
                    "value" in ec_en_frd.conditions["condition"] and ec_en_frd.conditions["condition"][
                "value"] != None) else " "
        cond_hal = (ec_en_hal.conditions["condition"]["value"]) if (
                    "value" in ec_en_hal.conditions["condition"] and ec_en_hal.conditions["condition"][
                "value"] != None) else " "
        cond_stj = (ec_en_stj.conditions["condition"]["value"]) if (
                    "value" in ec_en_stj.conditions["condition"] and ec_en_stj.conditions["condition"][
                "value"] != None) else " "
        ...
        hrly_period = [ec_en_wpg.hourly_forecasts[0]["period"], ec_en_wpg.hourly_forecasts[2]["period"],
                       ec_en_wpg.hourly_forecasts[4]["period"], ec_en_wpg.hourly_forecasts[6]["period"],
                       ec_en_wpg.hourly_forecasts[8]["period"], ec_en_wpg.hourly_forecasts[10]["period"],
                       ec_en_wpg.hourly_forecasts[12]["period"]]
        hrly_temp = [ec_en_wpg.hourly_forecasts[0]["temperature"], ec_en_wpg.hourly_forecasts[2]["temperature"],
                     ec_en_wpg.hourly_forecasts[4]["temperature"], ec_en_wpg.hourly_forecasts[6]["temperature"],
                     ec_en_wpg.hourly_forecasts[8]["temperature"], ec_en_wpg.hourly_forecasts[10]["temperature"],
                     ec_en_wpg.hourly_forecasts[12]["temperature"]]
        hrly_cond = [str(ec_en_wpg.hourly_forecasts[0]["condition"]), str(ec_en_wpg.hourly_forecasts[2]["condition"]),
                     str(ec_en_wpg.hourly_forecasts[4]["condition"]),
                     str(ec_en_wpg.hourly_forecasts[6]["condition"]), str(ec_en_wpg.hourly_forecasts[8]["condition"]),
                     str(ec_en_wpg.hourly_forecasts[10]["condition"]),
        ...
        s2 = "" + (str(hrly_period_local[0].strftime("%-I:%M %p"))).rjust(8, " ") + " " + str(local_tz) + "  " + (
            str(hrly_temp[0])).rjust(3, " ") + " C  " + hrly_cond[0][0:13]
        s3 = "" + (str(hrly_period_local[1].strftime("%-I:%M %p"))).rjust(8, " ") + " " + str(local_tz) + "  " + (
            str(hrly_temp[1])).rjust(3, " ") + " C  " + hrly_cond[1][0:13]
        s4 = "" + (str(hrly_period_local[2].strftime("%-I:%M %p"))).rjust(8, " ") + " " + str(local_tz) + "  " + (
            str(hrly_temp[2])).rjust(3, " ") + " C  " + hrly_cond[2][0:13]
        s5 = "" + (str(hrly_period_local[3].strftime("%-I:%M %p"))).rjust(8, " ") + " " + str(local_tz) + "  " + (
            str(hrly_temp[3])).rjust(3, " ") + " C  " + hrly_cond[3][0:13]
        s6 = "" + (str(hrly_period_local[4].strftime("%-I:%M %p"))).rjust(8, " ") + " " + str(local_tz) + "  " + (
            str(hrly_temp[4])).rjust(3, " ") + " C  " + hrly_cond[4][0:13]
        s7 = "" + (str(hrly_period_local[5].strftime("%-I:%M %p"))).rjust(8, " ") + " " + str(local_tz) + "  " + (
            str(hrly_temp[5])).rjust(3, " ") + " C  " + hrly_cond[5][0:13]
        s8 = "" + (str(hrly_period_local[6].strftime("%-I:%M %p"))).rjust(8, " ") + " " + str(local_tz) + "  " + (
            str(hrly_temp[6])).rjust(3, " ") + " C  " + hrly_cond[6][0:13]
        ...
        wpg_precip = (str(ec_en_wpg.conditions["pop"]["value"]) + " %") if ec_en_wpg.conditions["pop"] and \
                                                                           ec_en_wpg.conditions["pop"][
                                                                               "value"] != None else "NIL"
        brn_precip = (str(ec_en_brn.conditions["pop"]["value"]) + " %") if ec_en_brn.conditions["pop"] and \
                                                                           ec_en_brn.conditions["pop"][
                                                                               "value"] != None else "NIL"
        tps_precip = (str(ec_en_tps.conditions["pop"]["value"]) + " %") if ec_en_tps.conditions["pop"] and \
                                                                           ec_en_tps.conditions["pop"][
                                                                               "value"] != None else "NIL"
        fln_precip = (str(ec_en_fln.conditions["pop"]["value"]) + " %") if ec_en_fln.conditions["pop"] and \
                                                                           ec_en_fln.conditions["pop"][
                                                                               "value"] != None else "NIL"
        thm_precip = (str(ec_en_thm.conditions["pop"]["value"]) + " %") if ec_en_thm.conditions["pop"] and \
                                                                           ec_en_thm.conditions["pop"][
                                                                               "value"] != None else "NIL"
        chu_precip = (str(ec_en_chu.conditions["pop"]["value"]) + " %") if ec_en_chu.conditions["pop"] and \
                                                                           ec_en_chu.conditions["pop"][
                                                                               "value"] != None else "NIL"
        yest_precip = (str(ec_en_wpg.conditions["precip_yesterday"]["value"]) + " MM") if ec_en_wpg.conditions[
                                                                                              "precip_yesterday"] and \
                                                                                          ec_en_wpg.conditions[
                                                                                              "precip_yesterday"][
                                                                                              "value"] != None else "0.0 MM"
...
def weather_update(group):
    ...
    if (timechk > 1800) or (
            group == 0):  # check if 30min has elapsed since last group update, but always allow group 0 updates (initial refresh)
        ...
        if (group == 0 or group == 1):
            ...
            real_forecast_time = time.strftime(
                "%I %p")  # this is used as the forecast time when showing the weather. for some reason the dictionary was always reporting 22:00 for forecast time
            if real_forecast_time == "12 PM":
                real_forecast_time = "NOON"  # just to add some fun
            real_forecast_date = datetime.datetime.now().strftime(
                "%a %b %d/%Y")  # this is used as the forecast time when showing the weather. dictionary from env_canada reports weird (GMT maybe?)
        ...
        debug_msg(("WEATHER_UPDATE-weather group " + str(group) + " not updated. only ~" + str(
            round(timechk // 60)) + " minutes elapsed out of required 30"), 1)
...
def bottom_marquee(grouptotal):
    ...
    for n in range(len(wpg.entries)):
        if (n == 0) or ((len(wpg_desc + pad + wpg.entries[n][
            "description"]) * 24) >= 31000):  # avoid duplicate first entry / check if string will be max pixels allowed
            n = n + 1
        else:
...
root.geometry(
    "720x480")  # this must be 720x480 for a proper filled out screen on composite output. 640x480 will have black bar on RH side. use 720x576 for PAL.
root.config(cursor="none", bg="#00006F")
...
Title = Label(root, text="NATIONAL WEATHER SERVICE", font=("VCR OSD Mono", 22, "bold"), fg="white", bg="green")
...
# group 1
# weather_laca = current_weather(zipcode='91601')
weather_laca = current_weather(station='KBUR')[0]
uv_laca = feedparser.parse('https://data.epa.gov/efservice/getEnvirofactsUVHOURLY/ZIP/91601/xml')
